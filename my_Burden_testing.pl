...
while ( my $ID = <$INPUT> ){
    ...
    open (my $bedoutput, "> roi.bed");
    foreach my $line ($CollapsedBed){
        print $bedoutput $line, "\n";
    }
    #`sort -k1,1 -k2,2n tmp_filtered_regions.bed > filtered_regions.bed`;
    close $bedoutput;
...
sub BedToolsQuery {
    ...
    my $queryString = sprintf("intersectBed -wb -a <(echo -e \"chr%s\\t%s\\t%s\\t%s\") -b %s -sorted | cut -f9-",
                                $chr, $start, $end, $stable_ID, $geneBedFile);
...
sub FilterLines {
    ...
    # Saving temporary bedfile:
    open (my $tempbed, "> tmp_filtered_regions.bed");
